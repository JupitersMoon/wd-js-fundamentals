...
function weightedAverageMovieReview(rachel, roger, jake) {
 return ((rachel * 0.25) + roger + (jake * 2.2)) / 3;
}
...
function areaOfRing(outerRadius, innerRadius) {
 return ((3.14159 * (outerRadius * outerRadius)) - (3.14159 *(innerRadius * innerRadius)));
}
...
function longestOfThree(value1, value2, value3) {
  var longestWord = Math.max(value1.length, value2.length, value3.length);
  if (value1.length === longestWord) {
    return value1;
  } else if (value2.length === longestWord) {
    return value2;
  } else if (value3.length === longestWord) {
    return value3;
}
}
...
function capitalize (message) {
  ...
  var rest = message.slice(1);
  return firstCharacterUpper + rest;
}
